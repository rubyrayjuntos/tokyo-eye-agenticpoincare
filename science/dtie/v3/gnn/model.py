...
- version: v3
  ...
  change: Replace Euclidean cone depth with hyperbolic geodesic depth (`dist0`),
    ...
    flag, and attach full audit trail for depth-dependent downstream behavior.
...
class TopologicalMoEGate(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim: int,
        num_experts: int,
        blocked_experts: Optional[List[int]] = None,
        tau_gate: float = 1.0,
        inference_rho_vulnerable: float = 10.0,
        inference_block_expert_idx: Optional[int] = None,
    ):
        super().__init__()
        ...
        blocked = sorted({int(i) for i in (blocked_experts or []) if 0 <= int(i) < num_experts})
        self.blocked_experts = blocked
        self.tau_gate = max(float(tau_gate), 1e-4)
        self.inference_rho_vulnerable = float(inference_rho_vulnerable)
        self.inference_block_expert_idx = (
            int(inference_block_expert_idx)
            if inference_block_expert_idx is not None and 0 <= int(inference_block_expert_idx) < num_experts
            else None
        )
    ...
    def forward(
        ...
        if self.blocked_experts:
            mask = torch.zeros_like(logits, dtype=torch.bool)
            mask[:, self.blocked_experts] = True
            logits = logits.masked_fill(mask, float("-inf"))

        if (not self.training) and self.inference_block_expert_idx is not None and rho is not None:
            rho_vals = rho.view(-1)
            if rho_vals.numel() == logits.size(0):
                vulnerable = rho_vals < self.inference_rho_vulnerable
                if vulnerable.any():
                    logits[vulnerable, self.inference_block_expert_idx] = float("-inf")

        scores = F.softmax(logits / self.tau_gate, dim=-1)
...
class GOSPConeMapper(nn.Module):
    ...
    def __init__(
        ...
        self.gate = TopologicalMoEGate(
            hidden,
            num_experts,
            blocked_experts=blocked_experts,
            tau_gate=tau_gate,
            inference_rho_vulnerable=inference_rho_vulnerable,
            inference_block_expert_idx=inference_block_expert_idx,
        )
    ...
    def forward(self, data: Data) -> Dict[str, Any]:
        ...
        rho = data.x[:, 0:1] if (hasattr(data, "x") and data.x is not None and data.x.size(1) > 0) else None
        scores, balance_loss = self.gate(x, data.clustering, depth, rho=rho)
...
def gosp_loss(
    ...
    tokyo_loss_mode: str = "evidential",
    w_var_penalty: float = 0.0,
    aleatoric_hinge_target: float = 1.0,
    w_aleatoric_hinge: float = 0.0,
) -> Dict[str, Any]:
    """
    ...
    """
    mode = str(tokyo_loss_mode).strip().lower()
    if mode == "gaussianlikelihood":
        mode = "gaussian_likelihood"

    # Component 1: primary regression objective.
    if mode == "gaussian_likelihood":
        mu = output["evidence"]["mu"]
        aleatoric = torch.clamp(output["uncertainty"]["aleatoric"], min=1e-8)
        err = (target_rho - mu) ** 2
        ev_loss = (0.5 * torch.log(aleatoric) + 0.5 * err / aleatoric).mean()
    else:
    ...
    aleatoric = torch.clamp(output["uncertainty"]["aleatoric"], min=1e-8)
    regularity_mask = (1.0 - target_dehydron).clamp(min=0.0, max=1.0)
    var_penalty = (regularity_mask * aleatoric).mean()

    hinge_target = max(float(aleatoric_hinge_target), 1e-6)
    aleatoric_hinge = (regularity_mask * torch.relu(aleatoric - hinge_target).pow(2)).mean()

    total = (
        ev_loss
        + balance_coeff * bal_loss
        + cone_coeff * cone_loss
        + (w_var_penalty * var_penalty)
        + (w_aleatoric_hinge * aleatoric_hinge)
    )
