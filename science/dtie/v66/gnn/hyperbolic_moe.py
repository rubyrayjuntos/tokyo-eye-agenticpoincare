...
# Namespace for domain-separated gate/prototype init (independent of node_emb width).
_GATE_INIT_NAMESPACE = "hyperbolic_prototype_gate"
_BANK_INIT_NAMESPACE = "hyperbolic_prototype_bank"
    ...
    def __init__(
        ...
        *,
        ...
        if init_seed is not None:
            gen = torch.Generator()
            gen.manual_seed(derived_seed(_BANK_INIT_NAMESPACE, init_seed))
            self.prototype_tangent = nn.Parameter(
                init_scale * torch.randn(num_prototypes, dim, generator=gen)
            )
        else:
            self.prototype_tangent = nn.Parameter(
                init_scale * torch.randn(num_prototypes, dim)
            )
...
class HyperbolicPrototypeGate(nn.Module):
    ...
    TOPO_DIM = 8  # base board without SASA; instance ``topo_dim`` may be 9
    ...
    def __init__(
        ...
        self.topo_dim = self.TOPO_DIM + (1 if self.include_sasa else 0)
        ...
        # Isolate all gate RNG from earlier model construction (node_emb width, etc.).
        # Prototype bank also uses a private Generator keyed by seed so bank draws
        # stay stable even if Linear/Mobius order inside this block changes later.
        gate_ctx = (
            isolated_torch_seed(derived_seed(_GATE_INIT_NAMESPACE, init_seed))
            if init_seed is not None
            else nullcontext()
        )
        with gate_ctx:
            ...
            self.prototype_bank = HyperbolicPrototypeBank(
                num_experts,
                hidden_dim,
        ...
        self.register_buffer("sasa_mean", torch.tensor(0.0))
        self.register_buffer("sasa_var", torch.tensor(1.0))
    ...
    def _update_running_stats(
        ...
        with torch.no_grad():
            ...
            if self.include_sasa and sasa is not None:
                s_mean = sasa.mean()
                s_var = sasa.var(unbiased=False)
                if self.num_updates == 0:
                    self.sasa_mean.copy_(s_mean)
                    self.sasa_var.copy_(s_var)
                else:
                    self.sasa_mean.mul_(1 - momentum).add_(momentum * s_mean)
                    self.sasa_var.mul_(1 - momentum).add_(momentum * s_var)
    ...
    def _topo_features(
        ...
        if self.training:
            self._update_running_stats(log_degree, rho, sasa=sasa)
        ...
        if self.include_sasa:
            if sasa is None:
                raise ValueError("include_sasa=True requires sasa tensor in gate forward")
            sasa_f = sasa.float().reshape(-1)
            norm_sasa = (sasa_f - self.sasa_mean) / (self.sasa_var.sqrt() + 1e-8)
            parts.append(norm_sasa.unsqueeze(-1))
    ...
    def forward(
        ...
        *,
        ...
        # Raw gate hand-features (pre-MLP) for trunk-origin diagnostics.
        self._last_topo_features = topo.detach()
        ...
        # Pairwise prototype hyp distances (upper triangle) — nearest-pair repulsion.
        e = self.num_experts
        proto_i = proto_hyp.unsqueeze(0).expand(e, -1, -1)
        proto_j = proto_hyp.unsqueeze(1).expand(-1, e, -1)
        proto_pair = pmath.dist(proto_i, proto_j, k=k)
        tri_i, tri_j = torch.triu_indices(e, e, offset=1, device=proto_pair.device)
        proto_pair_ut = proto_pair[tri_i, tri_j]
        prototype_pair_min_dist = proto_pair_ut.min()
        # Unit-row Gram logdet (live) — saturating hinge applied in loss.
        # float32 required: torch.logdet does not support Half under AMP.
        proto_tan_u = F.normalize(
            self.prototype_bank.prototype_tangent.float(), dim=-1
        )
        gram = proto_tan_u @ proto_tan_u.T
        gram_eps = 1e-4
        eye = torch.eye(e, device=gram.device, dtype=torch.float32)
        prototype_gram_logdet = torch.logdet(gram.float() + gram_eps * eye)
        scale = F.softplus(self.logit_scale)
        floor = getattr(self, "logit_softplus_floor", None)
        if floor is not None:
            scale = torch.clamp(scale, min=float(floor))
        raw_logits = -scale * dists + self.expert_bias
        ...
        # Eval/diagnostic stash: pre-softmax distances + logits (overwritten each forward).
        self._last_pre_softmax = {
            "dists": dists.detach(),
            "raw_logits": raw_logits.detach(),
            "adjusted_logits": adjusted_logits.detach(),
            "fused_hyp": fused_hyp.detach(),
            "proto_hyp": proto_hyp.detach(),
            "topo_tangent": topo_tangent.detach(),
            "logit_scale": float(scale.detach()),
            "k": k.detach() if torch.is_tensor(k) else k,
        }
        ...
        audit: dict[str, Any] = {
            ...
            "prototype_pair_min_dist": prototype_pair_min_dist,
            "prototype_gram_logdet": prototype_gram_logdet,
            "topology_only": self.topology_only,
            "gate_disc_input": self.use_disc_position,
            "deep_gate": self.deep_gate,
            "logit_scale_softplus": float(scale.detach()),
        }
