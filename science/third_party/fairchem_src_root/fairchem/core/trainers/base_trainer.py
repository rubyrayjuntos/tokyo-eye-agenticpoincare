...
from fairchem.core.datasets.base_dataset import create_dataset, Subset
...
class BaseTrainer(ABC):
    ...
    def load_datasets(self) -> None:
        ...
        if "src" in self.config["dataset"]:
            ...
            if self.config["dataset"].get("max_atoms", None):
                indices = self.train_dataset.indices
                max_atoms = self.config["dataset"].get("max_atoms", None)
                if not self.train_dataset.metadata_hasattr("natoms"):
                    raise ValueError("Cannot use max_atoms without dataset metadata")
                indices = indices[self.train_dataset.get_metadata("natoms", indices) <= max_atoms]
