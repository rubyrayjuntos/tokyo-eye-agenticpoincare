"""Atomic runner: pharmacophore_identification (Act 04)."""
...
from science.compute.jobs.pharmacophore_identification import run_pharmacophore_identification
...
async def run_pharmacophore_identification_job(db: Any, ctx: JobRunContext) -> JobRunResult:
    """Run full pharmacophore identification as an atomic job (tier 2)."""
    ...
    phase_result = await run_pharmacophore_identification(
    ...
    if phase_result.success:
        try:
            await persist_phase_result(
                ...
                config=config,
                caller_identity="compute_job_pharmacophore_identification",
            )
            await db.commit()
        except Exception as exc:
            logger.exception("Pharmacophore persistence failed for %s", structure_id)
    ...
    return JobRunResult(
        ...
        artifacts_produced=["pharmacophores"] if phase_result.success else [],
