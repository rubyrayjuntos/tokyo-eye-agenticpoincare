"""
Tokyo Eye v7 — Hyp MP primary GNN
=================================
Eidetix Bio | 2026-07

Production module: science.tokyo_eye.TokyoEye
Lineage: v7 (hyperbolic neighbor message passing SSOT).
Seeded from frozen v6.6 architecture; diverges under science/tokyo_eye/ only.
Do not version this model under science/dtie/.
"""
...
from science.tokyo_eye.equivariant_conv_multirel import EquivariantConvMultiRel
from science.tokyo_eye.equivariant_conv_thermo import EquivariantConvThermo
from science.tokyo_eye.evidential import (
    DecoupledEvidentialHead,
    expand_coupled_uncertainty_state_dict,
    uncertainty_head_is_decoupled,
)
from science.tokyo_eye.hyperbolic_moe import (
...
from science.tokyo_eye.hyperbolic_mp import (
...
class TopologicalMoEGateV6(nn.Module):
    ...
    def __init__(
        ...
        if topology_only:
            gate_input_dim = (
                8  # clustering + cone_depth + log_degree + rho + tau + ss_onehot[3]
            )
        ...
        self.detach_gate_input = (
            False  # Set True in Phase 3 to freeze routing boundaries
        )
    ...
    def _update_running_stats(
        self, log_degree: torch.Tensor, rho: torch.Tensor
    ) -> None:
    ...
    def forward(
        ...
        if gumbel_active and self.training:
            ...
            scores = F.gumbel_softmax(
                adjusted_logits, tau=self.temperature, hard=True, dim=-1
            )
...
def infer_v66_model_kwargs(
    ...
    hyperbolic_expert_mix = bool(
        arch.get("hyperbolic_expert_mix", tc.get("hyperbolic_expert_mix", False))
    )
    topology_only = bool(
        arch.get("topology_only_gate", tc.get("topology_only_gate", False))
    )
    ...
    deep_hyperbolic_gate = bool(
        arch.get("deep_hyperbolic_gate", tc.get("deep_hyperbolic_gate", False))
    )
    ...
    gate_include_sasa = arch.get("gate_include_sasa", tc.get("gate_include_sasa"))
    if gate_include_sasa is None:
        topo_w = state_dict.get("gate.topo_encoder.0.weight")
        if topo_w is not None:
            in_f = int(topo_w.shape[1])
            disc = HyperbolicPrototypeGate.DISC_DIM if gate_disc_input else 0
            base = in_f - disc
            gate_include_sasa = base > HyperbolicPrototypeGate.TOPO_DIM
        else:
            gate_include_sasa = False
    else:
        gate_include_sasa = bool(gate_include_sasa)
    ...
    return {
        ...
        "structure_gate": bool(
            arch.get("structure_gate", tc.get("structure_gate", False))
        ),
        ...
        "radial_angular_recombine": str(
            arch.get(
                ...
                tc.get("radial_angular_recombine", "multiply"),
            )
        ),
        ...
        "uncertainty_from_backbone": bool(
            arch.get(
                "uncertainty_from_backbone", tc.get("uncertainty_from_backbone", False)
            )
        ),
        "thermo_edge_message_gate": bool(
            arch.get(
                "thermo_edge_message_gate",
                tc.get(
                    "thermo_edge_message_gate",
                    tc.get(
                        "thermo_edge_features",
                        any(
                            k.startswith("convs.0.thermo_gate") for k in state_dict
                        ),
                    ),
                ),
            )
        ),
        "multi_rel_edge_mp": bool(
            arch.get(
                "multi_rel_edge_mp",
                tc.get(
                    "multi_rel_edge_mp",
                    any(
                        k.startswith("convs.0.radial_mlps.") for k in state_dict
                    ),
                ),
            )
        ),
        "role_edge_mp": bool(
            arch.get(
                ...
                tc.get(
                    "role_edge_mp",
                    any(k.startswith("convs.0.radial_mlps.3.") for k in state_dict),
                ),
            )
        ),
        "role_coupling_edges": bool(
            arch.get(
                "role_coupling_edges",
                tc.get(
                    "role_coupling_edges",
                    any(k.startswith("convs.0.radial_mlps.4.") for k in state_dict),
                ),
            )
        ),
        "chem_edge_mp": bool(
            arch.get(
                ...
                tc.get(
                    "chem_edge_mp",
                    any(k.startswith("convs.0.radial_mlps.5.") for k in state_dict)
                    or any(k.startswith("convs.0.radial_mlps.6.") for k in state_dict),
                ),
            )
        ),
        "ha_edge_mp": bool(
        ...
        "dehydron_exclusivity": bool(
            arch.get(
                "dehydron_exclusivity",
                tc.get("dehydron_exclusivity", True),
            )
        ),
        "dehydron_angular_scale": float(
            arch.get(
                "dehydron_angular_scale",
                tc.get("dehydron_angular_scale", 1.0),
            )
        ),
        "rim_fanout_forward": bool(
            arch.get(
                "rim_fanout_forward",
                tc.get(
                    "rim_fanout_forward",
                    any(k.startswith("rim_fanout.") for k in state_dict),
                ),
            )
        ),
        "rim_fanout_strength": float(
            arch.get(
                "rim_fanout_strength",
                tc.get("rim_fanout_strength", 0.12),
            )
        ),
        "rim_fanout_min_r": float(
            arch.get(
                "rim_fanout_min_r",
                tc.get("rim_fanout_min_r", 0.35),
            )
        ),
        "spoke_edge_scale": float(
            arch.get(
                "spoke_edge_scale",
                tc.get("spoke_edge_scale", 1.0),
            )
        ),
        "ribbon_edge_scale": float(
            arch.get(
                "ribbon_edge_scale",
                tc.get("ribbon_edge_scale", 1.0),
            )
        ),
        "geometric_angular_prior": bool(
            arch.get(
                "geometric_angular_prior",
                tc.get("geometric_angular_prior", False),
            )
        ),
        "geometric_angular_kappa": float(
            arch.get(
                "geometric_angular_kappa",
                tc.get("geometric_angular_kappa", 1.0),
            )
        ),
        "geometric_angular_alpha": float(
            arch.get(
                "geometric_angular_alpha",
                tc.get("geometric_angular_alpha", 0.7853981633974483),
            )
        ),
    }
    ...
    *,
    ...
    if disc_projection_path is not None:
        ...
        if path not in ("pre_routing", "post_routing"):
            raise ValueError(
                f"disc_projection_path must be pre_routing or post_routing, got {path!r}"
            )
        return path
...
def infer_legacy_disc_projection_from_checkpoint(
    *,
    ...
    if (
        ...
        and metrics.get("disc_projection_source") == "pre_routing_x_hyp"
    ):
        return False
...
class TokyoEye(nn.Module):
    ...
    def __init__(
        ...
        thermo_edge_message_gate: bool = False,
        ...
        self.gate_include_sasa = bool(gate_include_sasa)
        self.init_seed = init_seed
        self.hyp_mp_primary = bool(hyp_mp_primary)
        self.se3_aux = bool(se3_aux) and not self.hyp_mp_primary
        # When Hyp MP is primary, SE(3) is optional aux only if explicitly requested
        # alongside hyp_mp (rare); default is Hyp MP alone.
        if bool(se3_aux) and self.hyp_mp_primary:
            self.se3_aux = True
        self.hyp_mp_layers = int(hyp_mp_layers)
        # Child lineage: Hyp MP on biology edges only (no Cα fallback).
        self.hyp_biology_mp = bool(hyp_biology_mp)
        # Core capacity quotas (optional; set from TrainingConfig / stage_runner).
        self.core_capacity_quota_tau: float = 0.0
        self._core_quota_dominant: dict[str, Any] = {}
        self._last_core_quota: dict[str, Any] | None = None
        ...
        self.dehydron_edge_barcode = bool(dehydron_edge_barcode)
        self.rim_fanout_forward = bool(rim_fanout_forward)
        self.rim_fanout_min_r = float(rim_fanout_min_r)
        self.spoke_edge_scale = float(spoke_edge_scale)
        self.ribbon_edge_scale = float(ribbon_edge_scale)
        self.geometric_angular_prior = bool(geometric_angular_prior)
        self.geometric_angular_kappa = float(geometric_angular_kappa)
        self.geometric_angular_alpha = float(geometric_angular_alpha)
        # Multi-rel / role supersedes the weak shared thermo gate.
        self.thermo_edge_message_gate = bool(thermo_edge_message_gate) and (
            not self.multi_rel_edge_mp
        )
        ...
        # SE(3) backbone — role / multi-rel typed paths or legacy thermo gate or geo-only
        if self.role_edge_mp:
            ...
            from science.tokyo_eye.thermo_edge_features import GEO_DIM as _GEO

            if self.chem_edge_mp:
                from science.tokyo_eye.chem_edge_graph import (
                ...
                spoke_rho_col = SPOKE_RHO_COL_CHEM
                coupling_strength_col = COUPLING_STRENGTH_COL_CHEM
                dehydron_barcode_col = DEHYDRON_BARCODE_COL_CHEM
            else:
                num_relations = (
                    NUM_ROLE_RELATIONS if self.role_coupling_edges else ROLE_COUPLING
                )
            ...
            if self.containment_edge_mp:
                from science.tokyo_eye.containment_edge_graph import (
                ...
                num_relations = NUM_ROLE_RELATIONS_WITH_CONTAINMENT
            ...
            if self.euclidean_shortcut_mp:
                from science.tokyo_eye.euclidean_shortcut_graph import (
            ...
            conv_kwargs = dict(
                ...
                spoke_edge_scale=self.spoke_edge_scale,
                ribbon_relation_id=ROLE_RIBBON,
                ribbon_edge_scale=self.ribbon_edge_scale,
            )
            if self.role_coupling_edges:
                conv_kwargs["coupling_strength_col"] = coupling_strength_col
                conv_kwargs["coupling_relation_id"] = ROLE_COUPLING
            if self.dehydron_edge_barcode:
                conv_kwargs["dehydron_barcode_col"] = dehydron_barcode_col
                conv_kwargs["dehydron_relation_id"] = ROLE_DEHYDRON
            ...
            if self.ha_edge_mp:
                ...
                from science.tokyo_eye.role_edge_graph import ROLE_PACKING
        ...
        elif self.multi_rel_edge_mp:
            self.convs = nn.ModuleList(
                [
                    EquivariantConvMultiRel(hidden, irreps_hidden="32x0e + 8x1e")
                    for _ in range(num_layers)
                ]
            )
        elif self.thermo_edge_message_gate:
            self.convs = nn.ModuleList(
                [
                    EquivariantConvThermo(hidden, irreps_hidden="32x0e + 8x1e")
                    for _ in range(num_layers)
                ]
            )
        else:
        ...
        # v7: Möbius neighbor MP (primary). SE(3) convs remain for optional aux.
        self.hyp_mp = (
            HyperbolicMessagePassingStack(hidden, num_layers=self.hyp_mp_layers)
            if self.hyp_mp_primary
            else None
        )
        ...
        if hyperbolic_gate:
            self.gate = HyperbolicPrototypeGate(
                ...
                include_sasa=self.gate_include_sasa,
                init_seed=init_seed,
            )
        else:
        ...
        head_cls = (
            DecoupledEvidentialHead if decoupled_uncertainty_heads else EvidentialHead
        )
        ...
        if self.rim_fanout_forward:
            from science.training.rim_fanout import RimFanoutSpread

            self.rim_fanout = RimFanoutSpread(
                spread_strength=rim_fanout_strength,
                min_r_rim=self.rim_fanout_min_r,
            )
        else:
            self.rim_fanout = None

        if self.geometric_angular_prior:
            from science.tokyo_eye.geometric_angular_prior import DiscAngularResidual

            self.disc_angular_residual = DiscAngularResidual(hidden)
        else:
            self.disc_angular_residual = None

    def _apply_rim_fanout(
        self,
        hyp_proj_2d_raw: torch.Tensor,
        data: Data,
    ) -> torch.Tensor:
        if self.rim_fanout is None:
            return hyp_proj_2d_raw
        ca = getattr(data, "ca_coords", None)
        return self.rim_fanout(hyp_proj_2d_raw, ca)
    ...
    def _tangent_from_radial_angular(
        ...
        if (
            self.radial_angular_recombine != "mlp_fusion"
            or self.radial_angular_fusion is None
        ):
            return cone
        fused_in = torch.cat([radial_depth, angular_direction], dim=-1)
    ...
    def forward(self, data: Data) -> dict[str, Any]:
        ...
        x_in = data.x
        if bool(getattr(self, "input_feature_zscore", False)) or bool(
            getattr(self, "replace_tau_with_abs_dist", False)
        ):
            from science.dtie.common.input_feature_norm import transform_node_features

            mean = getattr(self, "input_feat_mean", None)
            std = getattr(self, "input_feat_std", None)
            if mean is None or std is None:
                raise RuntimeError(
                    "input_feature_zscore/replace_tau_with_abs_dist set but "
                    "input_feat_mean/std missing"
                )
            x_in = transform_node_features(
                data.x,
                mean=mean,
                std=std,
                zscore=bool(getattr(self, "input_feature_zscore", False))
                or bool(getattr(self, "replace_tau_with_abs_dist", False)),
                replace_tau_with_abs_dist=bool(
                    getattr(self, "replace_tau_with_abs_dist", False)
                ),
            )
        x = self.node_emb(x_in)

        c = self.curvature
        k = -c

        # ── Step 2: Message passing ───────────────────────────────────────
        # v7 default: lift early → Möbius neighbor MP (primary transport).
        # Optional SE(3) aux runs in Euclidean before lift when se3_aux=True.
        if self.se3_aux or not self.hyp_mp_primary:
        ...
        structural_frozen = (
            bool(getattr(data, "structural_z_disc_frozen", False))
            and hasattr(data, "structural_z_disc")
            and data.structural_z_disc is not None
        )
        ...
        if structural_frozen:
            ...
            angular_direction = self.angular_head(x)
            tangent_vector = self._tangent_from_radial_angular(
                radial_depth, angular_direction
            )
            tangent_vector = rescale_tangent_before_expmap(tangent_vector, c)
            x_hyp = pmath.expmap0(tangent_vector, k=k)
            x_hyp, proj_count_s1, proj_frac_s1 = self._project_with_audit(x_hyp, k=k)

        biology_mp_audit = None
        ca_in_mp = False
        if self.hyp_mp_primary and self.hyp_mp is not None:
            allow_ca = False if self.hyp_biology_mp else None
            hyp_ei = resolve_hyp_mp_edges(data, allow_ca_fallback=allow_ca)
            if self.hyp_biology_mp:
                from science.tokyo_eye.biology_mp_audit import build_biology_mp_audit

                ca_in_mp = False
                biology_mp_audit = getattr(data, "biology_mp_audit", None)
                if biology_mp_audit is None:
                    biology_mp_audit = build_biology_mp_audit(
                        edge_index=hyp_ei,
                        edge_types=getattr(data, "biology_edge_types", None),
                        num_nodes=int(x_hyp.shape[0]),
                        ca_in_mp=False,
                        allow_ca_fallback=False,
                    )
            x_hyp = self.hyp_mp(x_hyp, hyp_ei, k)
            x_hyp, proj_count_mp, proj_frac_mp = self._project_with_audit(x_hyp, k=k)
            proj_count_s1 = proj_count_s1 + proj_count_mp
            proj_frac_s1 = 0.5 * (proj_frac_s1 + proj_frac_mp)
        ...
        hyp_proj_2d_pre_raw = self._apply_rim_fanout(hyp_proj_2d_pre_raw, data)
        geom_theta_prior = getattr(data, "geom_theta_prior", None)
        geom_theta_disc = None
        if (
            self.geometric_angular_prior
            and self.disc_angular_residual is not None
            and geom_theta_prior is not None
        ):
            from science.tokyo_eye.geometric_angular_prior import apply_geometric_disc_prior

            prior = geom_theta_prior.to(device=hyp_proj_2d_pre_raw.device, dtype=hyp_proj_2d_pre_raw.dtype)
            if prior.shape[0] != hyp_proj_2d_pre_raw.shape[0]:
                raise ValueError(
                    f"geom_theta_prior length ({prior.shape[0]}) != N "
                    f"({hyp_proj_2d_pre_raw.shape[0]})"
                )
            delta = self.disc_angular_residual(x)
            hyp_proj_2d_pre_raw, geom_theta_disc, _ = apply_geometric_disc_prior(
                hyp_proj_2d_pre_raw,
                prior,
                delta,
                alpha=self.geometric_angular_alpha,
            )
        elif self.geometric_angular_prior and geom_theta_prior is None:
            raise ValueError(
                "geometric_angular_prior=True but data.geom_theta_prior is missing; "
                "attach via prepare_training_batch / attach_geometric_angular_prior"
            )
        ...
        if self.hyperbolic_gate:
            ...
            gate_sasa_kw: dict[str, Any] = {}
            if self.gate_include_sasa:
                gate_sasa_kw["sasa"] = residue_sasa_from_data(data).squeeze(-1)
        ...
        # Core capacity quotas (GShard-style; dehydron-dominant second-choice ban).
        self._last_core_quota = None
        if float(getattr(self, "core_capacity_quota_tau", 0.0) or 0.0) > 0:
            from science.training.core_capacity_quota import apply_core_capacity_quota_ste

            pdb_key = str(getattr(data, "pdb_id", "?") or "?").upper()
            dominant = self._core_quota_dominant.get(pdb_key)
            if dominant is not None:
                dh = getattr(data, "dehydron", None)
                if dh is None:
                    dh = tau_flag
                scores, quota_stats = apply_core_capacity_quota_ste(
                    scores,
                    dh,
                    dominant.to(device=scores.device),
                    tau_cap=float(self.core_capacity_quota_tau),
                )
                quota_stats = {
                    **quota_stats,
                    "pdb_id": pdb_key,
                    "frozen_dominant": [
                        bool(x) for x in dominant.detach().cpu().tolist()
                    ],
                }
                self._last_core_quota = quota_stats
                gate_audit["core_quota"] = quota_stats
        ...
        unc_source = (
            x_tangent if self.uncertainty_from_backbone else x_routed_tangent_out
        )
        ...
        with torch.no_grad():
            hyp_proj_2d_post_legacy, _ = project_disc_2d_legacy(
                hyp_proj_2d_post_raw, k=k
            )
        ...
        binding_logits_pocket = self.binding_head_pocket(x_routed_tangent_out).squeeze(
            -1
        )
        binding_logits_interface = self.binding_head_interface(
            x_routed_tangent_out
        ).squeeze(-1)
        ...
        audit_trail = {
            ...
            if self.hyperbolic_gate
            else "topological_moe_specialization",
            "hyperbolic_gate": self.hyperbolic_gate,
            "hyperbolic_expert_mix": self.hyperbolic_expert_mix,
            "hyp_mp_primary": bool(getattr(self, "hyp_mp_primary", False)),
            "se3_aux": bool(getattr(self, "se3_aux", False)),
            "hyp_biology_mp": bool(getattr(self, "hyp_biology_mp", False)),
            "ca_in_mp": bool(ca_in_mp) if getattr(self, "hyp_biology_mp", False) else None,
            ...
            "structural_disc_layout": (
                getattr(data, "structural_disc_layout", None)
                if structural_frozen
                else None
            ),
            "disc_projection_path": self.disc_projection_path,
            "thermo_edge_message_gate": self.thermo_edge_message_gate,
            "multi_rel_edge_mp": self.multi_rel_edge_mp,
            ...
            "rim_fanout_forward": self.rim_fanout_forward,
            "spoke_edge_scale": self.spoke_edge_scale,
            "ribbon_edge_scale": self.ribbon_edge_scale,
            "geometric_angular_prior": self.geometric_angular_prior,
            "geometric_angular_kappa": self.geometric_angular_kappa,
            "geometric_angular_alpha": self.geometric_angular_alpha,
            "rim_fanout_strength": (
                float(self.rim_fanout.strength.detach().cpu())
                if self.rim_fanout is not None
                else None
            ),
            ...
            "disc_proj_softness": disc_softness
            if not self.legacy_disc_projection
            else 0.0,
            ...
            "uncertainty_input": "backbone"
            if self.uncertainty_from_backbone
            else "routed",
            ...
            "gate_input_dim": (
                (
                    int(getattr(self.gate, "topo_dim", HyperbolicPrototypeGate.TOPO_DIM))
                    + HyperbolicPrototypeGate.DISC_DIM
                )
                if getattr(self.gate, "use_disc_position", False)
                else (
                    int(getattr(self.gate, "topo_dim", HyperbolicPrototypeGate.TOPO_DIM))
            ...
            "gate_include_sasa": bool(self.gate_include_sasa),
            "affected_outputs": [
                "cone_depth",
                "cone_depth_routed",
                "cone_width",
        ...
        if biology_mp_audit is not None:
            audit_trail["biology_mp"] = biology_mp_audit
        ...
        return {
            ...
            "core_quota": self._last_core_quota,
            "x_hyp": x_hyp,
            ...
            "geom_theta_prior": geom_theta_prior,
            "geom_theta_disc": geom_theta_disc,
        }
...
def adapt_checkpoint_radial_mlps(
    state_dict: dict[str, torch.Tensor],
    model_state: dict[str, torch.Tensor],
) -> dict[str, torch.Tensor]:
    """Expand per-relation radial MLPs when resuming 4-rel → 5-rel checkpoints."""
    adapted = dict(state_dict)
    for key, new_t in model_state.items():
        if ".radial_mlps." not in key:
            continue
        old_t = adapted.get(key)
        if old_t is not None and old_t.shape == new_t.shape:
            continue
        # Copy from same layer's packing MLP (relation 0) when a new relation slot appears.
        parts = key.split(".")
        try:
            rel_idx = int(parts[parts.index("radial_mlps") + 1])
        except (ValueError, IndexError):
            continue
        donor_key = key.replace(f"radial_mlps.{rel_idx}.", "radial_mlps.0.")
        donor = adapted.get(donor_key)
        if donor is None or donor.shape != new_t.shape:
            continue
        adapted[key] = donor.clone()
    return adapted


def load_v66_state_dict(
    model: TokyoEye,
    state_dict: dict[str, torch.Tensor],
) -> tuple[list[str], list[str]]:
    """Load weights tolerating gate topo expansion and barcode node_emb widen."""
    from science.tokyo_eye.checkpoint_adapt import adapt_checkpoint_node_emb_width
    ...
    adapted = adapt_checkpoint_radial_mlps(adapted, model_state)
...
def verify_v66_checkpoint(
    checkpoint_path: str | None = None,
    *,
    device: str = "cpu",
) -> dict[str, Any]:
    """Verify checkpoint loads into the in-repo TokyoEye architecture.
    ...
    if resolved is None or not resolved.is_file():
        raise FileNotFoundError(f"TokyoEye checkpoint not found: {logical}")

    # Import gate module first — missing file fails fast with ImportError.
    from science.tokyo_eye import hyperbolic_moe  # noqa: F401
    ...
    kwargs = infer_v66_model_kwargs(state_dict, architecture, training_config)
    node_dim = int(kwargs.pop("node_dim", 4))
    model = TokyoEye(node_dim=node_dim, **kwargs)
    missing, unexpected = load_v66_state_dict(model, state_dict)
    ...
    return {
        ...
        "hyp_mp_primary": bool(kwargs.get("hyp_mp_primary", True)),
    }


# Public Tokyo Eye aliases (legacy ``*_v66_*`` names retained for checkpoint adapt).
infer_tokyo_eye_kwargs = infer_v66_model_kwargs
load_tokyo_eye_state_dict = load_v66_state_dict
# Lineage resume expects load_v7_state_dict / infer_v7_model_kwargs on the package module.
infer_v7_model_kwargs = infer_tokyo_eye_kwargs
load_v7_state_dict = load_tokyo_eye_state_dict


def verify_tokyo_eye_checkpoint(
    checkpoint_path: str | None = None,
    *,
    device: str = "cpu",
) -> dict[str, Any]:
    """Verify production TokyoEye checkpoint (health / CI)."""
    result = verify_v66_checkpoint(checkpoint_path, device=device)
    result["model_class"] = "TokyoEye"
    result["model_version"] = "TokyoEye-v7"
    return result
