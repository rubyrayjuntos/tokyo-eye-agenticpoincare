...
-- ============================================================================
-- ALTER: Add pgvector embedding and clustering_coefficient to fact_gnn_node_output
...
DO $$ BEGIN
    ALTER TABLE fact_gnn_node_output
        ADD COLUMN clustering_coefficient DOUBLE PRECISION;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;


-- ============================================================================
-- ALTER: Add run lifecycle columns to fact_dtie_run
-- Requirements: 7.1, 7.2, 7.3
-- ============================================================================

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN status TEXT DEFAULT 'running';
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN phase_timings JSONB DEFAULT '{}';
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN warnings JSONB DEFAULT '[]';
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN n_residues_gdp INTEGER;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN n_residues_gtp INTEGER;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN checkpoint_path TEXT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN n_landmarks INTEGER;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE fact_dtie_run ADD COLUMN effector_sites JSONB;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;


-- ============================================================================
-- FUNCTION: Poincaré distance in the Poincaré ball model
-- d(x,y) = (2/√c) * arctanh(√c * ||(-x) ⊕_c y||)
-- where ⊕_c is Möbius addition
-- Requirements: 4.2
-- ============================================================================

CREATE OR REPLACE FUNCTION poincare_distance(
    x vector(64),
    y vector(64),
    curvature_c DOUBLE PRECISION DEFAULT 1.0
) RETURNS DOUBLE PRECISION AS $$
DECLARE
    sqrt_c DOUBLE PRECISION;
    x_arr DOUBLE PRECISION[];
    y_arr DOUBLE PRECISION[];
    norm_x_sq DOUBLE PRECISION;
    norm_y_sq DOUBLE PRECISION;
    dot_neg_x_y DOUBLE PRECISION;
    num_coeff_x DOUBLE PRECISION;
    num_coeff_y DOUBLE PRECISION;
    denom DOUBLE PRECISION;
    norm_mob_sq DOUBLE PRECISION;
    norm_mob DOUBLE PRECISION;
    arg DOUBLE PRECISION;
    i INTEGER;
    dim INTEGER;
    mob_i DOUBLE PRECISION;
BEGIN
    sqrt_c := sqrt(greatest(curvature_c, 1e-6));
    x_arr := x::DOUBLE PRECISION[];
    y_arr := y::DOUBLE PRECISION[];
    dim := array_length(x_arr, 1);
    ...
    dot_neg_x_y := 0;
    FOR i IN 1..dim LOOP
        norm_x_sq := norm_x_sq + x_arr[i] * x_arr[i];
        norm_y_sq := norm_y_sq + y_arr[i] * y_arr[i];
        dot_neg_x_y := dot_neg_x_y + (-x_arr[i]) * y_arr[i];
    END LOOP;

    -- Möbius addition: (-x) ⊕_c y
    -- Numerator coefficients:
    --   coeff_x = 1 + 2c<-x,y> + c||y||²
    --   coeff_y = 1 - c||x||²
    -- Denominator: 1 + 2c<-x,y> + c²||x||²||y||²
    num_coeff_x := 1.0 + 2.0 * curvature_c * dot_neg_x_y + curvature_c * norm_y_sq;
    num_coeff_y := 1.0 - curvature_c * norm_x_sq;
    denom := 1.0 + 2.0 * curvature_c * dot_neg_x_y
             + curvature_c * curvature_c * norm_x_sq * norm_y_sq;
    denom := greatest(denom, 1e-12);

    -- Compute ||mob_add||²
    norm_mob_sq := 0;
    FOR i IN 1..dim LOOP
        mob_i := (num_coeff_x * (-x_arr[i]) + num_coeff_y * y_arr[i]) / denom;
        norm_mob_sq := norm_mob_sq + mob_i * mob_i;
    END LOOP;
    norm_mob := sqrt(greatest(norm_mob_sq, 0));

    -- d(x,y) = (2/√c) * arctanh(√c * ||(-x) ⊕_c y||)
    -- Clamp argument to arctanh domain (-1, 1)
    arg := sqrt_c * norm_mob;
    arg := least(arg, 0.99999);
    RETURN (2.0 / sqrt_c) * atanh(arg);
END;
$$ LANGUAGE plpgsql IMMUTABLE;


-- ============================================================================
-- ALTER: Add canonical_id to fact_dtie_doorway for Phase 2 extended persistence
-- Requirements: 5.2
-- ============================================================================

ALTER TABLE fact_dtie_doorway ADD COLUMN IF NOT EXISTS canonical_id TEXT;
