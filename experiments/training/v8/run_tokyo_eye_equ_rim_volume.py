"""Tokyo Eye EQU rim_volume — sealed 20-epoch volume + invariant lift train.

Fresh spine under pure-hyp freeze. No correct_start / champion / affinity / C1 θ.
L_vol + invariant projector; sealed sat/spread/equiv formulas per rim design.
"""
...
from experiments.training.v8.equ_rim_volume import (
    ...
    RimVolumeGuardError as CorrectStartGuardError,
    assert_frontend_bank,
    assert_not_forbidden_ckpt,
    assert_pure_hyp_strict,
    build_rim_volume_stamp as build_correct_start_stamp,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tokyo Eye EQU rim_volume")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    p.add_argument("--pins", type=Path, default=Path("data/gates/tokyo_eye_equ_rim_volume_pins.json"))
...
def main() -> int:
    ...
    run_name = f"eqf_equ_rim_volume_{date.today().strftime('%Y%m%d')}"
