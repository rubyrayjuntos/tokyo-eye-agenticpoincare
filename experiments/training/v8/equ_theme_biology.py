"""tokyo_eye_equ_theme_biology — theme dehydron AUPRC + geometry HOLD helpers."""
...
GATE_ID = "tokyo_eye_equ_theme_biology"
TAG = "[equ-theme-biology]"

# Re-export train knobs from lift_radius (same geometry recipe).
from experiments.training.v8.equ_lift_radius import (  # noqa: E402
    ...
    RV_VOLUME_SIGMA,
    RV_WEIGHT_DECAY,
    LiftRadiusGuardError as ThemeBiologyGuardError,
    assert_pure_hyp_strict,
    load_boot_split,
    normalized_routing_entropy,
)

DEFAULT_STAMP = Path("data/gates/tokyo_eye_equ_theme_biology.json")
DEFAULT_PINS = Path("data/gates/tokyo_eye_equ_theme_biology_pins.json")
MLFLOW_RUN_NAME = GATE_ID
CorrectStartGuardError = ThemeBiologyGuardError
...
DEFAULT_SPINE_INIT = Path(
    "checkpoints/tokyoeye/runs/eqf_equ_lift_radius_20260916/tokyoeye_best.pt"
)
...
def assert_not_forbidden_ckpt(path: Path | str) -> None:
    ...
    for token in FORBIDDEN_CKPT_SUBSTRINGS:
        if token and token in text:
            # Explicit spine_init from lift_radius QUALIFIED is allowed.
            if "lift_radius" in text and token in {
                "eqf_equ_rim_volume",
                "eqf_equ_shell_unpack",
                "eqf_equ_correct_start",
            }:
                continue
            if "lift_radius" in text and token in FORBIDDEN_CKPT_SUBSTRINGS:
                # only block if the path itself is a forbidden card run
                if any(
                    bad in text
                    for bad in (
                        "eqf_equ_rim_volume",
            ...
            if any(
                ...
                raise RuntimeError(f"forbidden ckpt token {token!r} in {text}")
    # Still apply lift/boot bank checks for random paths
    if "lift_radius" not in text:
    ...
    stamp.update(
        {
            ...
            "artifacts": [
                ...
                "docs/superpowers/plans/2026-09-15-tokyoeye-equ-theme-biology.md",
                "experiments/training/v8/run_tokyo_eye_equ_theme_biology.py",
            ],
        }
    )
    if extra:
...
__all__ = [
    ...
    "TB_THEME_COLLAPSE",
    "ThemeBiologyGuardError",
    "CorrectStartGuardError",
    "assert_frontend_bank",
    "assert_not_forbidden_ckpt",
    "assert_pure_hyp_strict",
    "build_theme_biology_stamp",
    "enrich_rim_probe_row",
    "epoch_is_best_eligible",
    "evaluate_lift_spine_equivariance",
    "evaluate_probe_hygiene",
