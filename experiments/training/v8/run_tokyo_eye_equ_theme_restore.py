"""Tokyo Eye EQU theme_restore — theme dehydron AUPRC on geoopt_restore spine.

Warm-start from QUALIFIED Equiformer-pool + geoopt best. Forbids SE(3)-lite
and theme_biology Fail θ. Geometry HOLD + biology NEW gates.
"""
...
from experiments.training.v8.equ_theme_restore import (
    ...
    ThemeRestoreGuardError as CorrectStartGuardError,
    assert_frontend_bank,
    assert_not_forbidden_ckpt,
    assert_pure_hyp_strict,
    build_theme_restore_stamp as build_correct_start_stamp,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tokyo Eye EQU theme_restore")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    p.add_argument("--pins", type=Path, default=Path("data/gates/tokyo_eye_equ_theme_restore_pins.json"))
    ...
    p.add_argument("--epochs", type=int, default=CS_EPOCHS)
    p.add_argument("--probe-every", type=int, default=CS_PROBE_EVERY)
    ...
    p.add_argument("--spine-init", type=Path, default=DEFAULT_SPINE_INIT,
                    help="QUALIFIED geoopt_restore best ckpt to warm-start")
...
def main() -> int:
    ...
    if args.allow_stub_frontend:
        raise CorrectStartGuardError("theme_restore forbids --allow-stub-frontend / SE(3)-lite")
    ...
    system, load_info = build_equiformer_pool_system(
        cfg,
        equiformer_ckpt=frontend_path,
        device=device,
        freeze_backbone=True,
    )
    if load_info.get("mode") != "equiformer_v3_pool_loaded":
        raise RuntimeError(f"unexpected frontend load: {load_info}")
    restore_pf = assert_geoopt_restore_preflight(system)
    print(f"{TAG} restore_preflight={restore_pf}", flush=True)
    ...
    require_learned_curvature(curvature, context="equ theme_restore c")
    ...
    if not args.fresh_spine:
        ...
        print(f"{TAG} fresh_spine=True (no geoopt_restore warm start)", flush=True)
    ...
    switch_lb_coeff = float(cfg.get("switch_lb_coeff", 1.0))
    soft_quota_coeff = float(cfg.get("soft_quota_coeff", 5.0))
    telemetry = dict(cfg.get("telemetry") or {})

    run_name = f"eqf_equ_theme_restore_{date.today().strftime('%Y%m%d')}"
            ...
            mlflow.set_tags(
                {
                    ...
                    "init": "geoopt_restore_qualified_warmstart",
    ...
    def _abort_stamp(reason: str, probe_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        biology = evaluate_theme_restore(probe_rows or [])
        ...
        for epoch in range(int(args.epochs)):
            ...
            for entry in train:
                ...
                def _step() -> dict[str, float]:
                    return run_epoch(
                        ...
                        switch_lb_coeff=switch_lb_coeff,
                        soft_quota_coeff=soft_quota_coeff,
                        telemetry=telemetry,
            ...
            if (epoch + 1) % int(args.probe_every) == 0:
                ...
                if use_mlflow and mlflow is not None:
                    ...
                    bio_mid = evaluate_theme_restore(probe_rows)
