"""Tokyo Eye EQU geoopt_restore — Equiformer-to-pool + geoopt lift train.

Cold spine under pure-hyp freeze. Forbids SE(3)-lite. Uses official EquiformerV3
cut before energy_block + geoopt PoincareBall lift. Geometry HOLD only (no biology).
"""
...
from experiments.training.v8.equ_geoopt_restore import (
    ...
    GeooptRestoreGuardError as CorrectStartGuardError,
    assert_frontend_bank,
    assert_geoopt_restore_preflight,
    assert_not_forbidden_ckpt,
    assert_pure_hyp_strict,
    build_geoopt_restore_stamp as build_correct_start_stamp,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tokyo Eye EQU geoopt_restore")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    p.add_argument("--pins", type=Path, default=Path("data/gates/tokyo_eye_equ_geoopt_restore_pins.json"))
...
def main() -> int:
    ...
    if args.allow_stub_frontend:
        raise CorrectStartGuardError(
            "geoopt_restore forbids --allow-stub-frontend / SE(3)-lite stubs"
        )
    ...
    if load_info.get("mode") != "equiformer_v3_pool_loaded":
        raise RuntimeError(f"unexpected frontend load (need equiformer_v3_pool_loaded): {load_info}")
    restore_pf = assert_geoopt_restore_preflight(system)
    print(f"{TAG} restore_preflight={restore_pf}")
    ...
    require_learned_curvature(curvature, context="equ geoopt_restore c")
    ...
    run_name = f"eqf_equ_geoopt_restore_{date.today().strftime('%Y%m%d')}"
            ...
            mlflow.set_tags(
                {
                    ...
                    "init": "equiformer_v3_pool_plus_fresh_spine",
