"""Tokyo Eye EQU shell_unpack — sealed 20-epoch volume + invariant lift train.

Fresh spine under pure-hyp freeze. No correct_start / champion / affinity / C1 θ.
L_rad + final_only τ clamp + invariant projector; sealed rim gate formulas.
"""
...
from experiments.training.v8.equ_shell_unpack import (
    ...
    ShellUnpackGuardError as CorrectStartGuardError,
    assert_frontend_bank,
    assert_not_forbidden_ckpt,
    assert_pure_hyp_strict,
    build_shell_unpack_stamp as build_correct_start_stamp,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tokyo Eye EQU shell_unpack")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    p.add_argument("--pins", type=Path, default=Path("data/gates/tokyo_eye_equ_shell_unpack_pins.json"))
...
def main() -> int:
    ...
    run_name = f"eqf_equ_shell_unpack_{date.today().strftime('%Y%m%d')}"
