"""Tokyo Eye EQU shell_unpack — gates, L_rad knobs, final_only τ clamp (CPU-safe)."""
...
GATE_ID = "tokyo_eye_equ_shell_unpack"
DISPLAY_LINEAGE = "Tokyo Eye EQU"
MLFLOW_EXPERIMENT = "tokyoeye/equiformer-v3-moe/geometric/hyperbolic-spine"
MLFLOW_RUN_NAME = "tokyo_eye_equ_shell_unpack"
DEFAULT_MANIFEST = Path("manifests/equ_cold_boot_v1.json")
DEFAULT_PINS = Path("data/gates/tokyo_eye_equ_shell_unpack_pins.json")
DEFAULT_STAMP = Path("data/gates/tokyo_eye_equ_shell_unpack.json")
...
class ShellUnpackGuardError(RuntimeError):
...
def assert_not_forbidden_ckpt(path: Path | str) -> None:
    ...
    for token in FORBIDDEN_CKPT_SUBSTRINGS:
        if token in text:
            raise ShellUnpackGuardError(f"forbidden checkpoint path token {token!r}: {p}")
    ...
    stamp.update(
        {
            ...
            "artifacts": [
                "docs/superpowers/specs/2026-09-15-tokyoeye-equ-shell-unpack-design.md",
                "docs/superpowers/plans/2026-09-15-tokyoeye-equ-shell-unpack.md",
                "experiments/training/v8/run_tokyo_eye_equ_shell_unpack.py",
            ],
        }
    )
    if extra:
...
__all__ = [
    ...
    "RV_TAU_CLAMP_MODE",
    "ShellUnpackGuardError",
    "assert_frontend_bank",
    "assert_not_forbidden_ckpt",
    "assert_pure_hyp_strict",
    "build_shell_unpack_stamp",
