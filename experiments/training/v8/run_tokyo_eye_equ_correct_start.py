"""Tokyo Eye EQU correct_start — sealed 20-epoch geometry train.

Cold restart under pure-hyp freeze. Does not load champion / affinity / C1.
Frontend frozen (geometry Pass; same GPU path as cold boot). Affinity parked.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tokyo Eye EQU correct_start")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    p.add_argument("--pins", type=Path, default=Path("data/gates/tokyo_eye_equ_correct_start_pins.json"))
...
def main() -> int:
    ...
    run_name = f"eqf_equ_correct_start_{date.today().strftime('%Y%m%d')}"
    ...
    try:
        ...
        for entry in probe:
            try:
                ...
                equiv_residual = evaluate_equivariance_residual(
                    system,
                    batch,
                    tau_ceiling=CS_TAU_END,
                    seed=int(args.seed),
                )
                print(f"{TAG} Δ_equiv={equiv_residual:.6e} on {entry['pdb_id']}:{entry['chain']}")
                break
            except Exception as exc:  # noqa: BLE001
