"""Tokyo Eye EQU rim_volume — gates, L_vol knobs, lift+spine equiv (CPU-safe)."""
...
GATE_ID = "tokyo_eye_equ_rim_volume"
DISPLAY_LINEAGE = "Tokyo Eye EQU"
MLFLOW_EXPERIMENT = "tokyoeye/equiformer-v3-moe/geometric/hyperbolic-spine"
MLFLOW_RUN_NAME = "tokyo_eye_equ_rim_volume"
DEFAULT_MANIFEST = Path("manifests/equ_cold_boot_v1.json")
DEFAULT_PINS = Path("data/gates/tokyo_eye_equ_rim_volume_pins.json")
DEFAULT_STAMP = Path("data/gates/tokyo_eye_equ_rim_volume.json")
...
class RimVolumeGuardError(RuntimeError):
...
def assert_not_forbidden_ckpt(path: Path | str) -> None:
    ...
    for token in FORBIDDEN_CKPT_SUBSTRINGS:
        if token in text:
            raise RimVolumeGuardError(f"forbidden checkpoint path token {token!r}: {p}")
    ...
    stamp.update(
        {
            ...
            "artifacts": [
                "docs/superpowers/specs/2026-09-15-tokyoeye-equ-rim-volume-design.md",
                "docs/superpowers/plans/2026-09-15-tokyoeye-equ-rim-volume.md",
                "experiments/training/v8/run_tokyo_eye_equ_rim_volume.py",
            ],
        }
    )
    if extra:
