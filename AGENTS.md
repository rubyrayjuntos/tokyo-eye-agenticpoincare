...
13. **TokyoEye — active trunk SSOT (2026-07-24):** Production model under [`science/tokyo_eye/`](science/tokyo_eye/). Equiformer/SE(3) frontend + hyperbolic spine + MoE. **Restore SSOT:** `models:/TokyoEye@champion` (staging: `@experimental`). Hierarchy in MLflow: `tokyoeye/equiformer-v3-moe/{domain}/{subsystem}` → run = capability+goal. Local `.pt` paths are cache only. Governance: [`docs/superpowers/specs/2026-07-23-tokyoeye-mlflow-governance-design.md`](docs/superpowers/specs/2026-07-23-tokyoeye-mlflow-governance-design.md). Prior Hyp-MP / `checkpoints/v7/` is **dead archaeology**.
