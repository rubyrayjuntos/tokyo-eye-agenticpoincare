...
**Purpose:** Hyperbolic geometry platform for protein allostery detection, source-leak identification, and agentic structural biology workflows.
...
A model using per-residue dehydron density (ρ) + structure/chemistry graph connectivity can surface biologically meaningful allosteric and vulnerability signals in oncogenic proteins (KRAS, etc.) when operating in hyperbolic geometry, without functional labels.
...
science/tokyo_eye/          Tokyo Eye model product (active trunk under lineage package); not versioned under dtie
science/dtie/               Structural biology (historical name; do not version the model here). Future rename → structural-biology
...
5. **Discovery Story compute path is production**: V3/V4 code exists for backward compat only. New pathway/orchestrator work uses the current Discovery Story compute path exclusively.
...
12. **Hyperbolic geometry by default**: All geometric computations run in hyperbolic (Poincaré) space unless the onboard contract marks an artifact or job as `euclidean` or `mixed`. The contract (`science/contracts/onboard_contract.yaml`) is the SSOT for which jobs and artifacts are hyperbolic. **Curvature is learned at model inference** (job id may still be labeled `gnn_inference` in legacy contract keys → `embedding_space.curvature`) and must passthrough to downstream hyperbolic jobs — never hardcode a numeric curvature. Enforcement defaults to `warning`; set `GEOMETRIC_ENFORCEMENT_LEVEL=error` and `GEOMETRIC_ENFORCEMENT_ERROR_JOBS` in CI/staging for critical jobs. See `science/contracts/README.md`.
13. **TokyoEye — active trunk SSOT (2026-07-24):** Production model under [`science/tokyo_eye/`](science/tokyo_eye/). Equiformer/SE(3) frontend + hyperbolic spine + MoE. **Restore SSOT:** `models:/TokyoEye@champion` (staging: `@experimental`). Hierarchy in MLflow: `tokyoeye/equiformer-v3-moe/{domain}/{subsystem}` → run = capability+goal. Local `.pt` paths are cache only. Durable bytes: GitHub Releases (`tokyoeye-eqf-<sha256_16>`); `@champion` cannot move without a matching Release. Governance: [`docs/superpowers/specs/2026-07-23-tokyoeye-mlflow-governance-design.md`](docs/superpowers/specs/2026-07-23-tokyoeye-mlflow-governance-design.md), vault: [`docs/superpowers/specs/2026-08-23-tokyoeye-github-release-vault-design.md`](docs/superpowers/specs/2026-08-23-tokyoeye-github-release-vault-design.md). Prior Hyp-MP / `checkpoints/v7/` is **dead archaeology**.
...
15. **Legacy lines frozen archaeology:** Sealed Fix-1 under `checkpoints/v66/` and Hyp-MP under `checkpoints/v7/` are compare-only. Chem-MVP **PARKED**: [`docs/specs/chem-mvp-reengage/README.md`](docs/specs/chem-mvp-reengage/README.md). Run names containing `s4` are historical labels, not a backbone version.
...
5. **Tokyo Eye is the production model** — active package under `science.tokyo_eye` via contract / model catalog. **Never open dead lineages as trunk.** Legacy `GOSPConeMapper-v*` / `gospc_v*` remain compare-only. Pathway/orchestrator code under `science/dtie/v5` is not the model trunk.
