...
def test_gosp_loss_includes_weighted_floor_when_coeff_positive() -> None:
    ...
    output = {
        ...
        "routing_load_floor": torch.tensor(0.0004, device=device),
        "routing_entropy": torch.tensor(1.3, device=device),
        "expert_load": torch.tensor([0.25, 0.25, 0.25, 0.25], device=device),
    ...
    with_floor = gosp_loss_v6(
        output, target, coords, routing_load_floor_coeff=10.0, **zeroed,
    )
    assert float(with_floor["routing_load_floor"]) == pytest.approx(0.004, rel=1e-5)
    assert float(with_floor["total"]) - float(without["total"]) == pytest.approx(0.004, rel=1e-5)
