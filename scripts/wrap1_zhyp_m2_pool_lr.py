#!/usr/bin/env python3
"""wrap=1 z_hyp M2 macro-F1 -- Equiformer-pool frontend, LR-isolation card.

Companion to ``scripts/wrap1_zhyp_g_fit.py --card m2`` (SE(3)-lite, sealed
``tokyo_eye_equ_wrap1_zhyp_m2_result.json``). That card's frontend was
SE(3)-lite; ``_se3_lite_forward`` only executes ``blocks[0]``, so it could
never test frontend-architecture capacity (see lesson
``L-se3-lite-frontend-1-of-7-blocks``, ``data/gates/tokyo_eye_equ_agenda.
json``). This card swaps in the real ``EquiformerPoolFrontend`` (cold_init,
no MPtrj checkpoint -- addendum SS2.3 architecture-only default) and holds it
fixed across two arms, varying only ``lr_frontend``:
...
>=0.40 -- see ``tokyo_eye_equ_wrap1_zhyp_m2_prereg.json`` for the reachability
citations these bars were locked against).

Unlike the lite card, this one builds a real (non-stub, non-off-path)
frontend -- ``assert_governed_assembly`` runs with
``allow_off_path_frontend=False``, so ``pure_hyp_pass`` is a live, enforced
gate here, not skipped. (Verified clean for this exact config ahead of time;
see ``scripts/diag_gradflow_pool_frontend.py --mode verify-pure-hyp``.)

Compute note (read before scoping a full run): benchmarked on a single 4GB
GPU (Quadro T1000), one Adam step over the full mean-over-11-train-structures
batch costs ~31.4s wall-clock (memory flat at 3.18GB peak over 25 consecutive
steps -- no fragmentation growth observed). At the sealed card's locked 400
steps: ~209 min/fold, ~42h for 12 folds x 1 arm, ~84h for 12 folds x 2 arms.
This is a multi-day single-GPU commitment at full fidelity -- see the prereg
for the scope actually authorized before running non-smoke.

Environment: needs requirements-diagnostics-pool-frontend.txt (+ torch-cluster
matching the PyG wheel index) beyond requirements-science.txt -- see that
file's header for the exact install commands.
"""
...
def run_one_fold(
    ...
    train_batches = [G._load_batch(t, device) for t in train_tags]
    hold_batch = G._load_batch(hold, device)
    ...
    try:
        if mlflow is not None:
            mlflow.set_tags({
                ...
                "dehydron_coeff": "0.0", "margin_coeff": "0.0",
    ...
    return {
        ...
        "moe_mode": "ablated", "sdrp_coeff": SDRP_COEFF, "dehydron_coeff": 0.0, "margin_coeff": 0.0,
...
def main() -> int:
    ...
    p.add_argument("--lr-frontend-arm", choices=sorted(LR_ARMS), required=True)
