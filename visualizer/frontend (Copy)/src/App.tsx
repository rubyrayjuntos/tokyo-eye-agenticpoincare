...
export default function App() {
  ...
  // Selected pocket for cross-panel dynamic views (e.g. allosteric control sub-graph)
  const [selectedPocketId, setSelectedPocketId] = useState<number | null>(null);

  // Global Biophysical Radar mode (persists across pocket/structure interactions until user explicitly toggles off)
  const [isRadarActive, setIsRadarActive] = useState<boolean>(false);

  // Therapeutic Compiler state (multi-pathway atlas + resistance topology data) for cross-component sharing (e.g. Poincaré layer)
  const [therapeuticCompilerState, setTherapeuticCompilerState] = useState<any | null>(null);

  // Unified collapse simulation state (fraction + goal) for immediate re-optimization on either change
  ...
  const emitDirective = useCallback(
    (directive: ViewportDirective) => {
      ...
      // Also update highlightedResidues for backward compatibility with viewers
      if (
        ...
        setIsRadarActive(false); // clear also dismisses radar
      }
    },
    []
  );

  // Connect to the backend WebSocket for real-time viewport directives
  useViewportSocket({
      ...
      <HydrationProvider>
        <div className="w-full h-screen flex flex-col bg-zinc-950 text-zinc-100 font-sans overflow-hidden">
          ...
          <div className="flex flex-1 min-h-0">
            {/* Left sidebar: agent telemetry */}
            <AgentTelemetryPanel />

            {/* Center: viz grid + toolbar + results table */}
            ...
            {/* Agent chat sidebar (overlays on right when open) */}
            <AgentChat
