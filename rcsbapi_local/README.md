...
### Package Modules
The `rcsb-api` package currently provides four primary Python modules:

- `rcsbapi.search`: Build and execute RCSB Search API queries (text, attribute, sequence, structure, and chemical search).
- `rcsbapi.data`: Build and execute GraphQL queries against the RCSB Data API.
- `rcsbapi.sequence`: Query sequence-coordinate alignments and annotations.
- `rcsbapi.model`: Retrieve model server resources (e.g., full structures, ligands, atoms, interactions, and assemblies).
...
- Search API: [search docs](https://rcsbapi.readthedocs.io/en/latest/search_api/quickstart.html)
- Data API: [data docs](https://rcsbapi.readthedocs.io/en/latest/data_api/quickstart.html)
- Sequence API: [sequence docs](https://rcsbapi.readthedocs.io/en/latest/seq_api/quickstart.html)
- Model API: [model docs](https://rcsbapi.readthedocs.io/en/latest/model_api/quickstart.html)

### Module Selection Guide
Use this quick decision tree to choose the right module(s):

1. Do you need to discover IDs first?
  - Yes: start with `rcsbapi.search`.
  - No: start with `rcsbapi.data`, `rcsbapi.sequence`, or `rcsbapi.model` depending on your output needs.

2. What output do you need?
  - Structured metadata fields (methods, organism, annotations, etc.): use `rcsbapi.data`.
  - Sequence alignment or sequence-coordinate annotations: use `rcsbapi.sequence`.
  - Coordinates, ligands, atom neighborhoods, interactions, assemblies: use `rcsbapi.model`.

3. Common module combinations:
  - Discovery workflow: `search -> data`.
  - Structure retrieval workflow: `search -> model`.
  - Sequence interpretation workflow: `search -> data -> sequence`.
  - Known-target deep dive: `data -> model` (or `sequence -> model` when sequence context drives structure analysis).

Practical rule of thumb:
- Use `search` to find candidates.
- Use `data` to describe candidates.
- Use `sequence` to interpret sequence-level context.
- Use `model` to inspect 3D structural context.
